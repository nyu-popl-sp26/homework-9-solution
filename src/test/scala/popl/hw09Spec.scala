...
class hw09Spec extends AnyFlatSpec:
  ...
  "typInfer" should "reject programs that try to assign non-assignable expressions" in {
    ...
    intercept[LocTypeError] {
      inferType("""
        let x = 3; 
        const f = function (ref x: Num) { return x; };
        f(x + 1)
        """)
    }
    intercept[LocTypeError] {
      inferType("""
        ...
        const f = function (name y: Num) {
            return (function (ref z: Num) { return z; })(y);
          };
        f(3)
        """)
    }
  ...
  "typeInfer" should "infer types of assignment expressions" in {
    val e1 = "let x = 3; x = x + 1"
    assert(inferType(e1) === TNumber)
    
    val e2 = "let x = 3; let y = 5; x = y = x"
    assert(inferType(e2) === TNumber)
    
    intercept[StaticTypeError] {
      inferType("let x = true; x = 3")
    }
  }
  ...
  "Call" should "handle parameter passing modes" in {
    ...
    val e3 = """
      let x = 1;
      const f = function(ref x: Num) { x = x + 1; return x; };
      f(x) + x
    """
